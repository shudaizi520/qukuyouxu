...
class ReleaseVersionTests(unittest.TestCase):
    def test_application_and_static_pages_use_release_1_4_17(self):
        from helper import __version__

        self.assertEqual("1.4.17", __version__)
        pages = []
        for name in ("playlists.html", "daily.html", "home.html", "mixes.html", "external.html", "settings.html", "status.html"):
            with self.subTest(name=name):
                ...
                pages.append(html)
                self.assertIn("?v=1.4.17", html)
                self.assertIn(">v1.4.17<", html)
    ...
    def test_accepts_exact_v_prefixed_application_version(self):
        ...
        self.assertEqual("1.4.17", check_release_version("v1.4.17", "1.4.17"))
