...
function expireSession(){state={authenticated:false,username:state.username,setup_required:false};showAuth();if($('loginUser')&&state.username)$('loginUser').value=state.username;announceLogout();}
...
async function syncProfile(){const r=await raw('/api/plex/profiles');if(!r.ok)return;const d=await r.json(),ids=(d.items||[]).filter(row=>row.enabled).map(row=>String(row.id||''));if(!ids.includes(profile())){const fallback=ids.includes(String(d.active_profile_id||''))?String(d.active_profile_id):String(ids[0]||'');setProfile(fallback);}}
...
window.PCHAuth={ready,request,post,status:()=>state,profile,setProfile,logout,expire:expireSession};
