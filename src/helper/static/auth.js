...
function announceLogout(){window.dispatchEvent(new Event('pch-auth-logout'));}
...
async function boot(){try{await status();if(state.authenticated){await syncProfile();showWorkspace();}else showAuth();}catch(e){showAuth();message(e.message,true);}finally{readyResolve(state);window.dispatchEvent(new CustomEvent('pch-auth-ready',{detail:state}));}}
...
async function logout(){try{await raw('/api/auth/logout',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});}catch{}state={authenticated:false,username:state.username,setup_required:false};showAuth();if($('loginUser')&&state.username)$('loginUser').value=state.username;announceLogout();}
window.PCHAuth={ready,request,post,status:()=>state,profile,setProfile,logout};
