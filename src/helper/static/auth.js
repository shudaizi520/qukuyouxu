...
const CACHEABLE_JSON_PATHS=new Set(['/api/playlists','/api/mixes/status','/api/external/sources','/api/plex/saved','/api/automation']);
let readyResolve;const ready=new Promise(r=>readyResolve=r);
function profile(){return sessionStorage.getItem(PROFILE_KEY)||'';}
function setProfile(value){const id=String(value||'').trim();if(id)sessionStorage.setItem(PROFILE_KEY,id);else sessionStorage.removeItem(PROFILE_KEY);window.PCHPageCache?.activate(id);window.dispatchEvent(new CustomEvent('pch-profile-change',{detail:{profile_id:id}}));if(EMBEDDED&&window.parent!==window)window.parent.postMessage({type:'pch-profile-selected',profile_id:id},location.origin);}
...
function announceLogout(){window.dispatchEvent(new Event('pch-auth-logout'));if(EMBEDDED)window.parent.postMessage({type:'pch-auth-logout'},location.origin);}
function expireSession(){window.PCHPageCache?.clear();state={authenticated:false,username:state.username,setup_required:false};showAuth();if($('loginUser')&&state.username)$('loginUser').value=state.username;announceLogout();}
...
async function request(path,method='GET',body,recovered=false){const opt={method,credentials:'same-origin',cache:'no-store',headers:{}};if(method==='GET'&&/^\/api\/playlists\/[^/]+\/[^/]+\/cover$/.test(path))opt.cache='default';const profileId=profile();if(profileId)opt.headers['X-Plex-Profile']=profileId;if(body!==undefined){opt.headers['Content-Type']='application/json';opt.body=JSON.stringify(body);}const r=await fetch(path,opt);if(!r.ok){let d={};try{d=await r.json();}catch{}if(d.code==='profile_unavailable'&&!recovered){await syncProfile();return request(path,method,body,true);}if(r.status===401){try{await status();}catch{}expireSession();}throw Error(d.error||'请求失败：'+r.status);}return r;}
async function post(path,body){return (await request(path,'POST',body)).json();}
async function syncProfile(){const r=await raw('/api/plex/profiles');if(!r.ok)return;const d=await r.json(),items=Array.isArray(d.items)?d.items:[],ids=items.filter(row=>row.enabled).map(row=>String(row.id||''));window.PCHPageCache?.setProfiles(state.username||'',items);if(!ids.includes(profile())){const fallback=ids.includes(String(d.active_profile_id||''))?String(d.active_profile_id):String(ids[0]||'');setProfile(fallback);}else window.PCHPageCache?.activate(profile());}
async function cachedJson(path,options={}){const settings={...options,load:async()=>await (await request(path,'GET')).json()},target=new URL(path,location.origin),allowed=target.origin===location.origin&&!target.search&&CACHEABLE_JSON_PATHS.has(target.pathname);if(!allowed||!window.PCHPageCache)return {value:await settings.load(),source:'network'};return window.PCHPageCache.cachedJson(target.pathname,settings);}
function invalidateCache(tags,profileId){window.PCHPageCache?.invalidate(tags,profileId);}
async function boot(){try{await status();if(state.authenticated){await syncProfile();showWorkspace();}else if(EMBEDDED)expireSession();else showAuth();}catch(e){showAuth();message(e.message,true);}finally{readyResolve(state);window.dispatchEvent(new CustomEvent('pch-auth-ready',{detail:state}));}}
async function login(event){event.preventDefault();message('');try{const r=await raw('/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:$('loginUser').value.trim(),password:$('loginPassword').value})});let d={};try{d=await r.json();}catch{}if(!r.ok)throw Error(d.error||'登录失败');$('loginPassword').value='';await status();await syncProfile();showWorkspace();window.dispatchEvent(new CustomEvent('pch-auth-login',{detail:state}));}catch(e){message(e.message,true);}}
async function setup(event){event.preventDefault();message('');const password=$('setupPassword').value,confirm=$('setupConfirm').value;if(password!==confirm){message('两次输入的密码不一致',true);return;}try{const r=await raw('/api/auth/setup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:$('setupUser').value.trim(),password,confirm_password:confirm})});let d={};try{d=await r.json();}catch{}if(!r.ok)throw Error(d.error||'账户建立失败');$('setupPassword').value='';$('setupConfirm').value='';await status();await syncProfile();showWorkspace();window.dispatchEvent(new CustomEvent('pch-auth-login',{detail:state}));}catch(e){message(e.message,true);}}
async function logout(){try{await raw('/api/auth/logout',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});}catch{}expireSession();}
window.PCHAuth={ready,request,post,cachedJson,invalidateCache,status:()=>state,profile,setProfile,logout,expire:expireSession};
