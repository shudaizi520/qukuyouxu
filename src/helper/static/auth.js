...
async function syncProfile(){const r=await raw('/api/plex/profiles');if(!r.ok)return;const d=await r.json(),ids=(d.items||[]).map(row=>String(row.id||''));if(!ids.includes(profile())){const fallback=ids.includes(String(d.active_profile_id||''))?String(d.active_profile_id):String(ids[0]||'');setProfile(fallback);}}
