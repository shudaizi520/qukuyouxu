...
三个自动计划统一位于“设置 → 自动任务”：每日推荐可选择每天的执行时间，智能歌单可选择 3、5、7、10、14 或 20 天及执行时间，新增歌曲整理可选择每天的执行时间。时间均为北京时间，设置对所有已启用且已完成相应首次发布的“Plex 用户 + 音乐资料库”生效；后台严格串行运行，不会同时扫描多个曲库把 NAS 占满。

## 外部歌单

进入“外部歌单”，可以粘贴 QQ 音乐或网易云音乐的公开歌单链接，也可以上传 M3U、TXT、CSV 文件。每次导入都只和当前“Plex 用户 + 音乐资料库”比较，账户和曲库之间完全隔离。
